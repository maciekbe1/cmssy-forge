...
export async function configureCommand(options: ConfigureOptions) {
  ...
  const answers = await inquirer.prompt([
    {
      ...
      default: options.apiUrl || existingConfig.apiUrl || 'https://api.cmssy.io/graphql',
    },
    {
      type: 'password',
      name: 'apiToken',
  ...
  console.log(chalk.white('  cmssy-forge deploy      # Publish to marketplace'));
  console.log(chalk.white('  cmssy-forge sync        # Pull blocks from Cmssy\n'));
}
